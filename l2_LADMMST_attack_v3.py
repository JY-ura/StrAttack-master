...
BINARY_SEARCH_STEPS = 6  # number of times to adjust the constant with binary search
...
class LADMMSTL2:
    ...
    def compare(self, one_hot_possibility,  target_label):
        if not isinstance(one_hot_possibility, (float, int, np.int64)):
            one_hot_possibility = np.copy(one_hot_possibility)
            if self.TARGETED:
                one_hot_possibility[target_label] -= self.CONFIDENCE
            else:
                one_hot_possibility[target_label] += self.CONFIDENCE
            one_hot_possibility = np.argmax(one_hot_possibility)
        if self.TARGETED:
            return one_hot_possibility == target_label
        else:
            return one_hot_possibility != target_label
    ...
    def gradient_descent(self, sess, model):
        ...
        def doit(imgs, labs, z, CONST):
            ...
            l2s, scores, nimg, z_grads, loss = sess.run([l2dist_real, output, newimg, gradtz, loss1])

            return l2s, scores, nimg, np.array(z_grads), loss
    ...
    def attack(self, imgs, targets):
        ...
        r = []
        rv = []
        print('The number of image:', len(imgs))
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            print("outerstep:",outer_step, o_bestl2, CONST)
            ...
            for iteration in range(1000):
                ...
                # z step
                l2s, scores, nimg, z_grads, loss = self.grad(imgs, labs, z, CONST)
                Sc = y + v
                ...
                v = v + y - z
                s = s + w - z
                l2s, scores, nimg, y_grads, loss = self.grad(imgs, labs, y, CONST)
